...
tf.compat.v1.disable_eager_execution()  # avoid error for eager execution


class OUActionNoise(object):  # based in Ornstein-Uhlenbeck process
    ...
    def __call__(self):
        x = (
            ...
            * np.random.normal(
                size=self.mu.shape
            )  # root square of delta t is used for scale down
        )
        self.x_prev = x
        return x
...
class Actor(object):
    def __init__(
        ...
        self.actor_gradients = list(
            map(
                lambda x: tf.compat.v1.div(x, self.batch_size),
    ...
    def build_network(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            dense2 = tf.compat.v1.layers.dense(
                layer1_activation,
                units=self.fc2_dims,
    ...
    def train(self, inputs, gradients):
        self.sess.run(
            self.optimize,
            feed_dict={self.input: inputs, self.action_gradient: gradients},
        )

    def save_checkpoint(self):
        print("... saving checkpoint ...")
        self.saver.save(self.sess, self.chekpoint_file)

    def load_checkpoint(self):
        print("... loading checkpoint ...")
        self.saver.restore(self.sess, self.chekpoint_file)
...
class Critic(object):
    ...
    def predict(self, inputs, actions):
        return self.sess.run(
            self.q, feed_dict={self.input: inputs, self.actions: actions}
        )
    ...
    def get_action_gradients(self, inputs, actions):
        return self.sess.run(
            ...
            feed_dict={self.input: inputs, self.actions: actions},
        )
...
class Agent(object):
    def __init__(
        ...
        self.actor = Actor(
            ...
            layer1_size,
            layer2_size,
            env.action_space.high,  # action_bound
        )

        self.critic = Critic(
            beta, n_actions, "Critic", input_dims, self.sess, layer1_size, layer2_size
        )
